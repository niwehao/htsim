/*
 * main.cpp  —  OCS + EcsBuffer HOHO routing
 *
 * Topology: 8 GPUs connected via 1 OCS switch
 *
 * EcsBuffer architecture:
 *   - Each GPU node has one EcsBuffer (Edge Circuit Switch Buffer)
 *   - BFS routing table: rt[node][dst][slice] = FORWARD or WAIT
 *   - FORWARD: send to current slot's peer via ocs_tx
 *   - WAIT: buffer with target_slice tag, forward at matching slice
 *   - OCS reconfigures every ~365 us (7 time slots per cycle)
 *
 * Route per packet: [EcsBuffer@src, rxQ_dst, AppSink_dst]
 *   - GPU txQueue serializes → sendOn → EcsBuffer@src
 *   - EcsBuffer routes (FORWARD/WAIT/LOCAL) hop-by-hop
 *   - Final delivery: EcsBuffer@dst → sendOn → rxQueue → AppSink
 */
                  ...
                  std::array<GpuNode*, 8>& gpus)
{
    ...
    // First hop: EcsBuffer at source node (handles HOHO routing)
    route->push_back(ocs->getBuffer(srcGpu));

    // Destination: rxQueue + AppSink (reached when EcsBuffer@dst calls sendOn)
...
int main() {
    ...
    std::cout << "=== MOE htsim Simulation (OCS + EcsBuffer HOHO) ===\n"
              << "  GPUs=" << NUM_GPU_NODES
              << " Switch=1 (OCS, EcsBuffer HOHO)"
              << " Layers=" << TOTAL_LAYERS
    ...
    // ---- Build routes: [EcsBuffer@src, rxQ_dst, AppSink_dst] ----
    ...
    // ---- Print route info ----
    std::cout << "Route structure (OCS + EcsBuffer HOHO):\n";
    for (int src = 1; src <= 8; src++) {
        for (int dst = 1; dst <= 8; dst++) {
            if (src == dst) continue;
            ...
            // Show BFS routing for each slice
            bool hasFwd = false;
            for (int s = 0; s < (int)NUM_SLOTS; s++) {
                auto action = topo->getAction(src, dst, s);
                if (action == DynOcsTopology::FORWARD) {
                    int peer = topo->getPeer(src, s);
                    if (peer != dst) {
                        if (!hasFwd) { std::cout << " relay@slice:"; hasFwd = true; }
                        std::cout << " s" << s << "(via" << peer
                                  << ",d=" << topo->getDistance(src, dst, s) << ")";
                    }
                }
            }
            std::cout << "\n";
    ...
    // ---- Print topology ----
    std::cout << "OCS + EcsBuffer HOHO Topology:\n"
              << "  8 GPUs, each with one EcsBuffer\n"
              << "  OCS time-division: 7 slices per cycle\n"
              << "  Each slice: one perfect matching (4 bidirectional pairs)\n"
              << "  BFS routing: hop-by-hop FORWARD/WAIT decisions\n"
              << "  Max " << 7 << " hops (time-expanded graph diameter)\n"
