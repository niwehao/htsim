/*
 * gpu_node_sync.cpp  —  512-GPU 同步调度版 GPU 节点实现
 *
 * 与 htsim_syn 逻辑一致:
 *   - 按预计算 schedule 定时发送, 无 burst
 *   - 无重传, ACK 仅用于验证和阶段完成
 *   - 阶段完成后 cleanupPhase() 释放状态
 */
...
void GpuNodeSync::startInference() {
    if (_nodeId&PRINT_MOD == 0 && LOG_LEVEL >= 1 )
        std::cout << "[Node " << _nodeId << "] INFERENCE START (SYNC) — "
                  << TOTAL_LAYERS << " layers, "
...
void GpuNodeSync::advancePhase() {
    if (_currentPhase == 0) {
        ...
        if (_currentLayer < (int)TOTAL_LAYERS) {
            ...
            if (_nodeId&PRINT_MOD == 0 && LOG_LEVEL >= 1) {
                std::cout << "[Node " << _nodeId << "] INFERENCE FINISHED (SYNC)  t="
                          << timeAsMs(EventList::now()) << " ms\n";
            }
            _done = true;
