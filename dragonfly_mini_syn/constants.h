/*
 * constants.h  —  Dragonfly 同步调度版
 *
 * 基于 dragonfly/constants.h, 添加同步调度所需的常量和日志控制。
 *
 * Dragonfly 参数:
...
static constexpr int      LOG_LEVEL             = 2;                    // 0=quiet, 1=phase, 2=detail
...
static constexpr simtime_picosec BUFFER_DELAY_PS   =   2ULL * 1000000000ULL; // 2 ms (同步版中不会被触发)
...
inline void printGlobalSummary() {
    ...
    std::string sep(60, '=');
    std::cout << "\n" << sep << "\n"
              << "    GLOBAL MOE COMMUNICATION STATS (Dragonfly Sync)\n" << sep << "\n"
