...
public:
    ...
    static constexpr int MAX_HOPS = 1;
        ...
        , _cycle((simtime_picosec)NUM_SLOTS * (slice_active + reconfig))
    {
        ...
        // Try loading cached routing table; compute + save if not found
        std::string cacheFile = "bfs_cache_N" + std::to_string(N)
                              + "_S" + std::to_string(S)
...
private:
    bool loadRoutingTable(const std::string& path) {
        ...
        f.read((char*)&hS, 4);
        f.read((char*)&hH, 4);
        if (!f || hN != N || hS != S || hH != MAX_HOPS) return false;

        // Verify peer table matches (ensures same schedule)
        for (int node = 1; node <= N; node++)
            for (int s = 0; s < S; s++) {
                int32_t p; f.read((char*)&p, 4);
                if (!f || p != _peer[node][s]) return false;
            }

        // Load _rt, _dist, _fwd_hops
        for (int node = 1; node <= N; node++)
            for (int dst = 1; dst <= N; dst++) {
                f.read((char*)_rt[node][dst].data(), S);
                f.read((char*)_dist[node][dst].data(), S * 2);
                f.read((char*)_fwd_hops[node][dst].data(), S);
            }

        if (!f) return false;
        std::cout << "  Loaded BFS routing table from " << path << "\n";
        return true;
    }

    void saveRoutingTable(const std::string& path) {
        std::ofstream f(path, std::ios::binary);
        if (!f) { std::cerr << "Warning: cannot save BFS cache to " << path << "\n"; return; }

        int N = _num_ports, S = _num_slices;
        int32_t hN = N, hS = S, hH = MAX_HOPS;
        f.write((char*)&hN, 4);
        f.write((char*)&hS, 4);
        f.write((char*)&hH, 4);

        // Save peer table for validation
        for (int node = 1; node <= N; node++)
            for (int s = 0; s < S; s++) {
                int32_t p = _peer[node][s];
                f.write((char*)&p, 4);
            }

        // Save _rt, _dist, _fwd_hops
        for (int node = 1; node <= N; node++)
            for (int dst = 1; dst <= N; dst++) {
                f.write((char*)_rt[node][dst].data(), S);
                f.write((char*)_dist[node][dst].data(), S * 2);
                f.write((char*)_fwd_hops[node][dst].data(), S);
            }

        std::cout << "  Saved BFS routing table to " << path << "\n";
    }
