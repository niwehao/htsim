/*
 * constants.h  —  512-GPU Leaf-Spine 版
 *
 ...
 * 约束: NUM_GPU_NODES / GPUS_PER_LEAF <= PORTS_PER_SWITCH
 *
 * 包含:
 *   §1  网络/MOE 常量 + 拓扑参数
 *   §2  自动路由 (基于拓扑规律)
...
static constexpr uint32_t PORTS_PER_SWITCH    = NUM_GPU_NODES/8;
...
static_assert(NUM_LEAVES <= PORTS_PER_SWITCH,
              "NUM_LEAVES exceeds Spine port count — need 3-level topology");
...
static constexpr simtime_picosec BUFFER_DELAY_PS   =  0.2*1000000000ULL;         // 2 ms
...
// ================================================================
//  §2  自动路由 — 基于 Leaf-Spine 拓扑规律
//
//  GPU 编号: 0 .. NUM_GPU_NODES-1  (0-indexed)
//
//  GPU g 的位置:
//    leaf_index = g / GPUS_PER_LEAF
//    local_port = g % GPUS_PER_LEAF     (Leaf 下行端口号)
//
//  Leaf 端口布局:
//    0 .. GPUS_PER_LEAF-1    : 下行 (连 GPU)
//    GPUS_PER_LEAF .. 31     : 上行 (端口 GPUS_PER_LEAF+s 连 Spine s)
//
//  Spine 端口布局:
//    0 .. NUM_LEAVES-1       : 端口 l 连 Leaf l
//
//  路由规则:
//    同 Leaf:  GPU → Leaf → GPU               (1 switch, 5 hops in route)
//    跨 Leaf:  GPU → Leaf → Spine → Leaf → GPU (3 switches, 15 hops in route)
//
//  ECMP: spine_index = (src + dst) % NUM_SPINES
// ================================================================
...
inline uint32_t leafUpPort(uint32_t spineIdx) {
    return GPUS_PER_LEAF + spineIdx;  // 16 + spineIdx
}
