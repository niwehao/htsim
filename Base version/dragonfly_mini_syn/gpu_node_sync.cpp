/*
 * gpu_node_sync.cpp  —  Dragonfly 同步调度版 GPU 节点实现
 *
 * 与 dragonfly/gpu_node.cpp 的区别:
 *   - 按预计算 schedule 定时发送 (非 burst)
 *   - 无重传 (调度保证零拥塞)
 *   - ACK 仅用于阶段完成判断
 */
...
void GpuNodeSync::startInference() {
    if (LOG_LEVEL >= 1)
        std::cout << "[Node " << (int)_nodeId << "] INFERENCE START (Dragonfly Sync) -- "
                  << TOTAL_LAYERS << " layers, "
...
void GpuNodeSync::advancePhase() {
    if (_currentPhase == 0) {
        ...
        if (_currentLayer < (int)TOTAL_LAYERS) {
            ...
            if (LOG_LEVEL >= 1)
                std::cout << "[Node " << (int)_nodeId << "] INFERENCE FINISHED (Dragonfly Sync)  t="
